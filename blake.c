...
void rounds(uint32_t m[16])
{
	...
	{
		// column steps
		g(&state32[0][0], &state32[1][0], &state32[2][0], &state32[3][0], round, i++, m);
		g(&state32[0][1], &state32[1][1], &state32[2][1], &state32[3][1], round, i++, m);
		g(&state32[0][2], &state32[1][2], &state32[2][2], &state32[3][2], round, i++, m);
		g(&state32[0][3], &state32[1][3], &state32[2][3], &state32[3][3], round, i++, m);
		
		// diagonal steps
		g(&state32[0][0], &state32[1][1], &state32[2][2], &state32[3][3], round, i++, m);
		g(&state32[0][1], &state32[1][2], &state32[2][3], &state32[3][0], round, i++, m);
		g(&state32[0][2], &state32[1][3], &state32[2][0], &state32[3][1], round, i++, m);
		g(&state32[0][3], &state32[1][0], &state32[2][1], &state32[3][2], round, i++, m);
...
char *blake256(unsigned char *message, unsigned len)
{
	uint32_t i;
	uint32_t h[8];
	uint32_t s[4] = { 0x00, 0x00, 0x00, 0x00 };
	uint32_t t[2];
	uint32_t m[16];
	...
	*mp_len = 0x01;
	...
	if((newLen % 512) == 0){
	   nBlocks = newLen / 512;
		...
		printf(stderr, "Fuck shit deu merda da grossa!");
	}
